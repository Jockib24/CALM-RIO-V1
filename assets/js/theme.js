...
(function($) {
    ...
    //===== Preloader fallback (if window.load fails)
    setTimeout(function(){
    ...
    //===== Nice select js (exclude language switcher)
    
    if ($.fn.niceSelect && $('select:not(.lang-select)').length){
        $('select:not(.lang-select)').niceSelect();
    }
